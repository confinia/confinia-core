...
# OPS_DSN by container name: the ops database publishes no host port since
# 2026-08-12 (platform audit). 127.0.0.1 inside a container is the
# container's own loopback, so a loopback DSN would break silently.
# (Comment moved out of the podman run: a # line inside a backslash
...
podman run -d --name confinia-sbx_api --network host --restart unless-stopped \
  ...
  python -m uvicorn main:app --host 127.0.0.1 --port 11420
